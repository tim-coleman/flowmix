...
cv_aggregate <- function(destin,
  ...
  stopifnot(exists("nfold"))
  stopifnot(exists("cv_gridsize"))
  stopifnot(exists(c("lambda_pis")))
  stopifnot(exists(c("lambdas")))
      ...
      for(ifold in 1:nfold){
        for(irep in 1:nrep){
          ...
          tryCatch({
            ...
            cvscore.array[iprob, imu, ifold, irep] = cvscore
  ...
  rownames(cvscore.mat) = signif(lambda_pis,3)
  colnames(cvscore.mat) = signif(lambdas,3)
